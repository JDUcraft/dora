...
export default function Home() {
  ...
  return (
    <>
      <div className="fixed top-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-sm py-3 px-4 shadow-sm border-b border-slate-200">
        <div className="max-w-7xl mx-auto">
          <div className="flex justify-between items-start gap-2 mb-2">
            <h1 className="text-base md:text-xl lg:text-2xl font-bold text-slate-900 leading-tight flex-1">
              {t('home.title')}
            </h1>
            <select
              value={locale}
              onChange={(e) => changeLocale(e.target.value as 'fr' | 'en')}
              className="text-xs md:text-sm font-semibold text-slate-700 bg-white border border-slate-300 rounded-lg px-2 py-1.5 md:px-3 md:py-2 hover:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent cursor-pointer shrink-0"
            >
              <option value="fr">🇫🇷 FR</option>
              <option value="en">🇬🇧 EN</option>
            </select>
          ...
          <div className="text-xs md:text-sm font-semibold text-slate-600">
            {t('common.progression')}: {Object.keys(answers).length}/{questions.length}
          </div>
        </div>
      </div>
